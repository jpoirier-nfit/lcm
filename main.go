...
// tryConnectDocker attempts to connect to Docker using multiple socket paths
func tryConnectDocker(ctx context.Context) (*client.Client, string, error) {
	// List of socket paths to try, in order of preference
	socketPaths := []string{
		"",                                      // Try DOCKER_HOST env var first (empty means use FromEnv)
		"unix:///var/run/docker.sock",          // Standard Docker Desktop
		"unix://" + os.Getenv("HOME") + "/.rd/docker.sock",            // Rancher Desktop (older)
		"unix://" + os.Getenv("HOME") + "/.docker/run/docker.sock",    // Rancher Desktop (newer)
	}
	...
	for _, socketPath := range socketPaths {
		...
		if displayPath == "" {
			displayPath = os.Getenv("DOCKER_HOST")
			if displayPath == "" {
				displayPath = "default (DOCKER_HOST or /var/run/docker.sock)"
			}
		}
		return cli, displayPath, nil
	}

	return nil, "", fmt.Errorf("failed to connect to Docker/Rancher: %v", lastErr)
}

func main() {
	// Initialize Docker client - try multiple socket paths
	ctx := context.Background()
	cli, socketPath, err := tryConnectDocker(ctx)
	if err != nil {
		fmt.Printf("Error: Cannot connect to Docker or Rancher Desktop.\n")
		fmt.Printf("Tried the following:\n")
		fmt.Printf("  - DOCKER_HOST environment variable\n")
		fmt.Printf("  - /var/run/docker.sock (Docker Desktop)\n")
		fmt.Printf("  - ~/.rd/docker.sock (Rancher Desktop)\n")
		fmt.Printf("  - ~/.docker/run/docker.sock (Rancher Desktop)\n")
		fmt.Printf("\nError: %v\n\n", err)
		fmt.Printf("Please ensure Docker Desktop or Rancher Desktop is running.\n")
		os.Exit(1)
	}
	defer cli.Close()
...
func (m Model) viewListMode() string {
	...
	if m.socketPath != "" {
		// Simplify display of socket path
		displaySocket := m.socketPath
		if strings.Contains(displaySocket, ".rd/docker.sock") {
			displaySocket = "Rancher Desktop"
		} else if strings.Contains(displaySocket, ".docker/run/docker.sock") {
			displaySocket = "Rancher Desktop"
		} else if strings.Contains(displaySocket, "/var/run/docker.sock") {
			displaySocket = "Docker Desktop"
		}
		title += " [" + displaySocket + "]"
	}
	...
	if m.loading {
		...
		// Account for: title(2) + header(2) + status(2) + filter(2) + help(5) = ~13 lines overhead
		...
		// Find the longest NAME, IMAGE, STATE, and STATUS in the current view
		...
		for i := startIdx; i < endIdx; i++ {
			...
			if len(c.State) > maxStateLen {
				maxStateLen = len(c.State)
			}
		...
		imageWidth := maxImageLen + 4
		stateWidth := maxStateLen + 4
		statusWidth := maxStatusLen + 4

		// Calculate if we need to constrain widths to fit terminal
		// Fixed columns: ID(12) + cursor(2) + spacing(10) = 24
		usedWidth := 24 + nameWidth + imageWidth + stateWidth + statusWidth

		if usedWidth > m.width {
			// Need to constrain - prioritize NAME, STATE, and STATUS, truncate IMAGE
			availableWidth := m.width - 24 - nameWidth - stateWidth - statusWidth
			if availableWidth < 10 {
				// Too narrow, need to truncate NAME too
				availableForNameImage := m.width - 24 - stateWidth - statusWidth
				nameWidth = availableForNameImage / 2
				imageWidth = availableForNameImage / 2
				if nameWidth < 10 {
					nameWidth = 10
				}
				if imageWidth < 10 {
		...
		// Header - styled with full width
		headerText := fmt.Sprintf(" %-12s  %-*s  %-*s  %-*s  %s ",
			"ID", nameWidth, "NAME", imageWidth, "IMAGE", stateWidth, "STATE", "STATUS")
		// Pad header to full width
		headerPadding := m.width - len(headerText)
		if headerPadding > 0 {
		...
		for i := startIdx; i < endIdx; i++ {
			...
			line := fmt.Sprintf(" %-12s  %-*s  %-*s  %-*s  %-*s",
				c.ID, nameWidth, name, imageWidth, image, stateWidth, stateText, statusWidth, c.Status)

			// Pad line to full terminal width
			linePadding := m.width - len(line) - 2 // -2 for cursor indicator
			if linePadding > 0 {
	...
	// Filter status - styled box
	var filters []string
	if m.hideK8s {
		k8sCount := 0
		for _, c := range m.allContainers {
			if strings.HasPrefix(c.Name, "k8s_") {
				k8sCount++
			}
		}
		filters = append(filters, fmt.Sprintf("K8s: %d hidden "+keyStyle.Render("[h]"), k8sCount))
	}
	if m.hideExited {
		exitedCount := 0
		for _, c := range m.allContainers {
		...
		filters = append(filters, fmt.Sprintf("Exited: %d hidden "+keyStyle.Render("[a]"), exitedCount))
	}
	if len(filters) > 0 {
		filterText := "Active Filters: " + strings.Join(filters, " | ")
		s.WriteString(filterStyle.Render(filterText))
	}

	// Help text - styled box with highlighted keys
	helpText := "Controls:\n"
	helpText += fmt.Sprintf("  Navigation: %s Up  %s Down\n",
		keyStyle.Render("↑/k:"), keyStyle.Render("↓/j:"))
	helpText += fmt.Sprintf("  Actions:    %s Start  %s Stop  %s Restart  %s Shell\n",
		keyStyle.Render("s:"), keyStyle.Render("t:"), keyStyle.Render("R:"), keyStyle.Render("e/x:"))
